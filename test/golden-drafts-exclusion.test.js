...
// cockpit sitting, plus AUTHORED candidate files (e.g. the sci-033-class red-team corpus,
// drafts-sci033-class-*.jsonl) whose labels are PROVISIONAL until the operator ratifies
// them. Neither may ever ride along in an eval run: they have no adjudicated ground truth,
// they burn spend (calibration #4 burned ~$2 exactly this way), and a provisional label
// leaking into calibration numbers would corrupt the auto-air eligibility math. run.js
// excludes them by filename prefix (drafts-); this test pins that behavior functionally
// (via the exported loadGolden) so a refactor can't silently widen the glob — and it
// asserts the new sci-033-class draft file is covered by the exclusion.
...
test("loadGolden never loads any drafts-*.jsonl row", () => {
  const drafts = allDraftRows();
  assert.ok(drafts.length > 0, "drafts staging present — precondition for the exclusion check");
