...
- [PLANNED — P1-D, Decision D3] **Calibration-gated eligibility.** Auto-air eligibility becomes per-claim-category, earned by measured precision against a golden set of adjudicated claims. A category (e.g., historical dates, economic statistics) qualifies only when measured precision clears the bar; categories that haven't earned it route to the human regardless of confidence.
- [PLANNED — P1-B, Decision D4] **`person_private` carve-out.** Claims about named private individuals are structurally excluded from auto-air, permanently. No calibration result unlocks them.
