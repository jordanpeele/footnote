...
- **[HOW_FOOTNOTE_DECIDES.md](HOW_FOOTNOTE_DECIDES.md)** — the editorial policy. What counts as a checkable claim, the four-tier source hierarchy, verdict–evidence floors, and the **two-mode auto-air architecture (D19)**: VERIFIED — the default, earned stack (calibrated category, concurrence verifier, evidence rules, session cap) — and OPEN, where every check airs after a 2-second abort window wearing a visible **AI · UNVERIFIED** marker. Claims about named living individuals never auto-air in any mode (D4 — absolute). Plus the corrections rule (wrong verdicts get corrected on air, not deleted). Every rule is marked enforced-in-code or planned; divergence between code and an unmarked rule is a bug.
- **[docs/STREET_PROTOCOL.md](docs/STREET_PROTOCOL.md)** — the operator's one-page rulebook for live sessions, updated for the D19 modes: VERIFIED is the default posture; OPEN is a deliberate, disclosed choice; person-claims hold everywhere; machine-aired cards are permanently marked either way.
