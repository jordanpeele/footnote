<!-- extractor prompt v4 — 2026-08-18.
Changed from v3: MULTI-CLAIM output. Rapid-fire speech packs several distinct claims into
one transcript window, and the single-claim contract silently dropped all but one (the
2026-08-18 desk session lost "a million Haitian immigrants move to the UK every year" to a
neighboring claim sharing its window). Output is now {"claims":[...]} — up to 4, spoken
order, no repeats; NONE semantics unchanged. The per-claim field rules are verbatim from
v3: polarity / harm_class / category behavior per claim is unchanged.
v3 was — 2026-08-13.
Changed from v2: added "category" (closed six-value topical set), originally so the D18
pilot's category scope could be enforced in CODE (R57). R72 (2026-08-18) removed the
category gate — the field is now classification metadata for the session record and
operator display only. Unknown/missing category parses to "other". All other behavior
unchanged from v2.
v2 was — 2026-08-07.
